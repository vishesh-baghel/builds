 ...
 *
 * The deploy names its variables with a `SIFT_` prefix so one Vercel account can host several
 * builds off the same vendors without them reading each other's keys. Locally the unprefixed
 * names still work, because that is what the TypeSafe SDK and every other project on this
 * machine already use, and having to rename a key to run a script is friction with nothing
 * behind it.
 *
 * Prefixed wins where both are set.
 */
const read = (name: string): string | undefined => {
  const prefixed = process.env[`SIFT_${name}`];
