...
- **Not first-party API calls, so the model-specific parameter traps do not apply.** No
  `output_config.effort`, no `thinking` config, no per-model prompt-cache minimum to reason about, `generateObject` with a Zod schema is the whole interface. This is part of why the gateway is a
