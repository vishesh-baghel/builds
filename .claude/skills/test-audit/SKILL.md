---
name: test-audit
description: "builds repo only. Invoke whenever writing, changing, reviewing, or sweeping tests. Authoring gate for new tests plus audit workflow for low-value, implementation-coupled, or duplicative tests and the test-only production seams they demand."
---
...
whole subsystem's test surface (every test file one build or `shared` owns);
...
routing, and relevant history. Read the root `CLAUDE.md` first.
...
- the spine (`shared/`);
- each build directory (`reckon/`, `sift/`, `tally/`, and any newer one);
- UI, scripts, and tooling (`*/app/`, `scripts/`);
...
cross-language, package, release, or architecture contract. In this repo that
includes the six-stage spine, the four reliability primitives, and the shape of
the audit record (see `CLAUDE.md`). Also keep:
...
1. Run the smallest owner and sibling tests with
   `pnpm --filter @builds/<pkg> exec vitest run <path-or-filter>`.
...
4. Run `/validate-local`, scoped to the changed packages, which runs the
   repo gate from `CLAUDE.md`.
5. Inspect `git diff --numstat`; report production/tooling separately from
   tests and test support.
6. After final audit edits and the PR is open, run `/review-pr`.

## Landing and continuation

Commit, push, open a PR, or land only when authorized. Never
push past a red gate. Land one
...
(github.com/openclaw/openclaw, `.agents/skills/test-audit`), MIT License,
Copyright (c) 2026 OpenClaw Foundation. Tool and path references were changed
to this repo's; the method is unchanged.
