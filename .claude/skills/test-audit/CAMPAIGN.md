...
Campaign mode prunes one subsystem's whole test surface in one PR: one build
such as `reckon`, or the `shared` spine. The value bar, retention bar,
...
list: in the upstream Telegram campaign this method comes from, all three were real delivery bugs, not stale
tests.
...
Put durable test-ownership rules in `CLAUDE.md`, drawn from mistakes this
...
Record decisions that diverge from the plan in `docs/VARIANCE-LOG.md` and the
PR evidence rather than editing gates.
